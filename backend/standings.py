...
def build_standings(schedule: pd.DataFrame, year: int) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    results["PointsEarned"] = results.apply(
        lambda row: points_for_position(int(row["Position"] or 0), sprint=bool(row["Sprint"])),
        axis=1,
    )
    ...
    team_points = results.groupby(["TeamId", "TeamName"], dropna=False)["PointsEarned"].sum().reset_index()
    if max_pos > 0:
        team_pos_counts = (
            pos_df.groupby(["TeamId", "TeamName", "Pos"]).size().unstack(fill_value=0)
        )
        for i in range(1, max_pos + 1):
            if i not in team_pos_counts.columns:
                team_pos_counts[i] = 0
        team_pos_counts = team_pos_counts.reindex(team_points.set_index(["TeamId", "TeamName"]).index, fill_value=0)
        for i in range(1, max_pos + 1):
...
def main() -> None:
    ...
    # Export full standings to CSV files
    driver_csv = "standings_drivers.csv"
    constructor_csv = "standings_constructors.csv"
    driver_standings.to_csv(driver_csv, index=False)
    constructor_standings.to_csv(constructor_csv, index=False)
